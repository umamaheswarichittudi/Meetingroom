...
//     WHERE room_id = ?
//       AND (
//         (start_time < ? AND end_time > ?)  -- booking starts before new booking ends
//         OR (start_time < ? AND end_time > ?) -- booking ends after new booking starts
//         OR (start_time >= ? AND end_time <= ?) -- booking fully inside
//       )
//   `;
...
//       // Insert booking
//       const insertBooking = `
//         INSERT INTO bookings (room_id, username, start_time, end_time)
//         VALUES (?, ?, ?, ?)
//       `;
//       db.query(insertBooking, [roomId, username, start, end], (err2) => {
//         if (err2) return res.status(500).json({ message: err2.message });
//         res.json({
